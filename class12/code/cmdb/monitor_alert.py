...
DEFAULT_ADMIN = '717810674@qq.com'


policies = {
    'cpu' : {'count' : 3,'ceil' : 1},
    'mem' : {'count' : 3,'ceil' : 10},
    'disk': {'count' : 3,'ceil' : 25},
}
...
def monitor_alert():
    while True:
        ...
        _,rt_list = dbutils.execute_sql(SELECT_MONITOR_HOST,(r_time,),True)
        rt_dict = {}
        ...
        for key in rt_dict:
            ...
            for resource in SQL_SELECT_MONITOR_HOST_COLUMNS.values():
                if rt_dict[key][resource] > policies[resource]['count']:
                    messages.append(u'%s超过%s分钟内%s次数超过阈值%s%%' % (resource, INTERVAL_FETCH, policies[resource]['count'], policies[resource]['ceil']))
                ...
                if messages:
                    ...
                    mail.send_mail(admin,'<br/>'.join(messages),u'主机资源监控')
